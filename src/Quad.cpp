...
void batchDrawGrass(std::vector<GrassQuad>& quads, Shader& shader, Texture texture, Texture normalMap) {
    ...
    if (requiredBufferSize > currentBufferSize) {
        ...
        glBindBuffer(GL_ARRAY_BUFFER, g_QuadsVBO);
        glBufferData(GL_ARRAY_BUFFER, newBufferSize, NULL, GL_DYNAMIC_DRAW); // Resizing buffer
    ...
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(Vertex) * g_AllVertices.size(), g_AllVertices.data());
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, quads.size() * QUAD_INDEX_COUNT * sizeof(GLuint), indices, GL_STATIC_DRAW);
    ...
    glDrawElements(GL_TRIANGLES, (GLsizei)quads.size() * QUAD_INDEX_COUNT, GL_UNSIGNED_INT, 0);
